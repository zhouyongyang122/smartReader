...
public class SRNetManager {

    private static SRNetManager instance;

    private SRRequestApi sRxjavaApi;

    private Map<String, Boolean> mCancelMap = new HashMap<>();

    private SRNetConfig mNetConfig;

    private SRNetManager() {
        mNetConfig = new SRNetConfig();
    }

    public static SRNetManager shareInstance() {
        if (instance == null) {
            instance = new SRNetManager();
        }
        return instance;
    }
    ...
    public OkHttpClient.Builder getOkHttpBuilder() {
        return new OkHttpClient.Builder()
                ...
                .addInterceptor(new HttpLoggingInterceptor(new SRLog())
     ...
     * @return
     */
    public SRRequestApi getApi() {
        if (sRxjavaApi == null) {
            Retrofit retrofit = new Retrofit.Builder()
                    .client(getOkHttpBuilder().build())
                    .baseUrl(SRNetConfig.API_URL)
                    .addConverterFactory(GsonConverterFactory.create())
                    .addCallAdapterFactory(RxJavaCallAdapterFactory.create())
                    .build();

            sRxjavaApi = retrofit.create(SRRequestApi.class);
        }
        return sRxjavaApi;
    }
...
//        if (params != null) {
//            long time = System.currentTimeMillis() / 1000L + FZPreferenceHelper.getInstance().getTimeOffset();
//            params.put(FZApiKeyConstants.TIMESTAMP, time + "");
//
//            Map<String, String> tmp = new HashMap<>(params);
//            tmp.put(FZApiKeyConstants.SECURITY_KEY, FZAppConstants.SECURITY_KEY);
...
//            }
//            params.put(FZApiKeyConstants.SIGN, FZStringUtils.toMd5(sb.toString()));
//        }
//    }
